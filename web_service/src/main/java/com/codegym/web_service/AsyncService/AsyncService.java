...
public class AsyncService {
    ...
    @Async
    public void sendRegisterEmail(MemberDTO memberDTO) throws MessagingException {
        UserDetails userDetails = accountServiceImpl.loadAccountByAccountName(memberDTO.getAccountName());
        ...
        String htmlMsg = "<html>"
                ...
                + "</div>"
                + "<div style='margin: 1% 20%'>"
                + "<h3 style='font-size: 30px;'>Xin chào " + memberDTO.getUserName() + " !</h3>"
                + "<br>"
                + "<p style='font-size: 20px'>To confirm your account, please click here :"

                + " http://localhost:8080/confirm-account?token=" + confirmToken +"</p>"
                + "<p style='font-size: 20px'>Vui lòng truy cập trang để biết thêm chi tiết.</p>"
                + "<a href='http://localhost:4200/'>"
                + "<button style='padding: 10px;border: none; background : rgb(238, 159, 31);border-radius: 15px;color: #581008; cursor: pointer'>"
                + "<h2>CODEBAKERY</h2>"
        ...
        helper.setSubject("CodeBakery - Thông tin tài khoản");
        this.emailSender.send(message);
    }
    @Async
